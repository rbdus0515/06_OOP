...
public class User {
	...
	// ** 매개변수 : 생성자나 매서드 호출시 () 안에 작성되어
	// 				 전달되어지는 값을 저장하는 변수
	// -> 전달 받은 값을 저장하고 있는 매개체(지니고있는) 역할의 변수
	
	public User(String userId, String userPw) {
		
		System.out.println("매개변수 생성자를 이용해서 User 객채 생성");
	...
	public User(String userId, String userPw, String userName, int userAge,
		...
		// 생성자 내에서 반드시 첫번째 줄에 작성되어야 한다! 위에 다른 코드 쓰면 에러!
		// Constructor call must be the first statement in a constructor
		// -> 생성자 호출은 반드시 첫번째 줄에 작성해야만 한다.
		// 왜 사용하는가?
		// 중복코드 제거, 코드길이 감소, 재사용성 증가
		// 가독성이 좀 떨어지는 경우가 있어서 많이 사용안함.
		
		
		this(userId, userPw); // 이자식 위에 코드 있으면 에러남.
		// this.userId = userId;
		// this.userPw = userPw;
