...
public class ClsService /*extends Student*/{
	...
	public void ex2() {
		...
		// 정보출력
		System.out.println(std1.schoolName); // public이기때문에 직접 접근 가능
		...
		/* static
		 * 
		 * 1) 공유 메모리 영역(또는 적정 메모리 영역 이라고 함.
		 * 
		 *  why? 프로그램 시작 시 static이 붙인 코드들이 모두 static영역에 생성되고
		 *  프로그램 종료될 때 까지 사라지지 않음(정적메모리,공유메모리 영역)
		 *  그리고, static영역에 생성된 변수는 어디서든지
		 *  
		 *  2) 사용 방법 : 클래스명.변수명
		 *  
		 */
	...
	public void ex3() {
		...
		// 2) 매개변수 생성자를 이용해서 객체가 생성될 때 부터 다른값으로 필드 초기화
		
		User u3 = new User("test3", "password333"); // 매개변수 생성자
		// 생성자 수행 시 전달 할 값을 작성 (순서 꼭 지키기!!)
